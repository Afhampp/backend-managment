...
const postassigment=async(req,res)=>{
  ...
  const updatemark = async (req, res) => {
    try {
      ...
      if (!assignment) {
        return res.status(404).json({ error: 'Assignment or submission not found' });
      }
  
     
      const submission = assignment.submittion.find(sub => sub._id.toString() === submissionId);
    ...
    try {
      ...
      const modifiedData = getdata.map(assignment => {
        return {
          ...
          submittion: assignment.submittion.map(submission => {
